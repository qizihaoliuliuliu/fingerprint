...
u8 Pwd[6]="123456";  //解锁密码1
u8 Pwd2[6]="123456";  //解锁密码2
...
int press_FR(void)
{
	...
	{	
		...
		{		
			...
			{				
				LCD_Fill(0,100,lcddev.width,160,WHITE);
				Show_Str_Mid(0,100,"刷指纹成功，解锁中",16,240);		
				//Walkmotor_ON();				
				LCD_Fill(0,100,lcddev.width,160,WHITE);
				Show_Str_Mid(0,100,"已解锁",16,240);		
...
void Del_FR(void)
{
	...
	//Show_Str_Mid(0,120,"请验证管理员身份",16,240);
...
{    
	...
	while(1)
	{
		...
		if(PS_Sta)	 //检测PS_Sta状态，如果有手指按下
		{
			...
			a=press_FR();//刷指纹			
			LCD_Fill(0,100,lcddev.width,160,WHITE);	
		  str1=mymalloc(SRAMIN,20);
			sprintf(str1,"指纹为%d",a);
			Show_Str_Mid(0,100,(u8*)str1,16,240);
			myfree(SRAMIN,str1);
...
{        
    ...
    LCD_Fill(0,100,lcddev.width,160,WHITE);
		Show_Str_Mid(0,100,"ATK-ESP WIFI-STA 模式",16,240);
		LCD_Fill(0,120,lcddev.width,160,WHITE);
		Show_Str_Mid(0,120,"正在配置ATK-ESP8266模块，请稍等...",16,240);
	...
	PRESTA:
		...
		if(netpro&0X01)     //TCP Client    透传模式测试
		{
			LCD_Fill(0,100,lcddev.width,160,WHITE);
			LCD_Fill(0,120,lcddev.width,160,WHITE);
		  Show_Str_Mid(0,100,"ATK-ESP WIFI-STA 测试",16,240);
			Show_Str_Mid(0,120,"正在配置ATK-ESP模块,请稍等...",16,240);
			...
			{
					LCD_Fill(0,100,lcddev.width,160,WHITE);
					Show_Str_Mid(0,100,"WK_UP:返回重选",16,240);
				  LCD_Fill(0,120,lcddev.width,160,WHITE);
					Show_Str_Mid(0,120,"ATK-ESP 连接TCP失败",16,240);
					key=KEY_Scan(0);
